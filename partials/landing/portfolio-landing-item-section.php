<section class="portfolio-landing-item-section landing-item-section">
  <div class="portfolio-landing-item-container">
    <div class="title-wrapper">
      <h3>نمونه کار های ما</h3>
      <div class="audio-wrapper">
        <div class="voice-field">
          <div class="player">
            <div class="border-frame"></div>
            <div class="progress-border"></div>

            <button>
              <p>توضیحات بیشتر</p>
              <!-- آیکون قبل از پخش -->
              <svg id="play-icon" viewBox="0 0 330 330">
                <path d="M37.728,328.12c2.266,1.256,4.77,1.88,7.272,1.88c2.763,0,5.522-0.763,7.95-2.28l240-149.999
                        c4.386-2.741,7.05-7.548,7.05-12.72c0-5.172-2.664-9.979-7.05-12.72L52.95,2.28c-4.625-2.891-10.453-3.043-15.222-0.4
                        C32.959,4.524,30,9.547,30,15v300C30,320.453,32.959,325.476,37.728,328.12z" fill="black"></path>
              </svg>
            </button>

            <?php
            $portfolio_audio_url = get_post_meta(get_the_ID(), '_landing_portfolio_audio_url', true);
            if (!empty($portfolio_audio_url)) {
              echo '<audio src="' . esc_url($portfolio_audio_url) . '" preload="none"></audio>';
            } else {
              echo '<audio preload="none"></audio>';
            }
            ?>
          </div>
        </div>
      </div>
    </div>

    <div class="swiper portfolioSwiper">
      <div class="swiper-wrapper">
        <?php get_template_part('loop/landing/portfolio-landing-item-section', 'portfolio-landing-item-section') ?>
      </div>
    </div>
    <div class="button-wrapper">
      <a href="">مشاهده همه نمونه کار ها</a>
    </div>
  </div>
</section>