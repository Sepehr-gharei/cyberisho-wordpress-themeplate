jQuery(document).ready(function ($) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          form.append(
            '<p class="success-message">درخواست شما با موفقیت ارسال شد!</p>'
          );
          form[0].reset();
        } else {
          form.append(
            '<p class="error-message">' +
              (response.data || "خطای ناشناخته") +
              "</p>"
          );
        }
      },
      error: function (error) {
        ...
        form.append(
          '<p class="error-message">خطا در ارسال درخواست: ' +
            (error.responseText || "خطای ناشناخته") +
            "</p>"
        );
      },
    });
  ...
  $("#new-meeting-form-id").submit(function (e) {
    e.preventDefault();
    var form = $(this);
    // Clear honeypot field
    form.find('input[name="family"]').val("");
    ...
    $.ajax({
      ...
      success: function (response) {
        console.log("New Meeting Form Response:", response);
        if (response.success) {
          form.find('input[type="submit"]').hide();
          form.append(
            '<p class="success-message">درخواست شما با موفقیت ارسال شد!</p>'
      ...
      error: function (error) {
        console.log("New Meeting Form Error:", error);
        form.append(
          '<p class="error-message">خطا در ارسال درخواست: ' +
            (error.responseText || "خطای ناشناخته") +
            "</p>"
