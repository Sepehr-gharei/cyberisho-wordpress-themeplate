  ...
  $("#content a").on("click", function (e) {
    ...
    if (target.length) {
      $("html, body").animate(
        ...
        800
      ); // 800 میلی‌ثانیه = نیم ثانیه
    }
  });
});

jQuery(document).ready(function ($) {
  const sections = $("#toc-data").data("sections");

  if (sections && sections.length > 0) {
    let listHtml = '<div class="list-content-wrapper normal-content-wrapper">';
    listHtml += "<h5>فهرست مطالب</h5>";
    listHtml += "<ul>";

    sections.forEach(function (section) {
      if (section.header_content) {
        listHtml += "<li>" + section.header_content + "</li>";
      }
    });

    listHtml += "</ul></div>";

    $(".your-target-container").html(listHtml); // جایی که می‌خواهید فهرست نمایش داده شود
  }
});
