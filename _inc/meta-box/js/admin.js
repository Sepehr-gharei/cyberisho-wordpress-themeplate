jQuery(document).ready(function ($) {
    // Single Audio Upload
    $(document).on("click", ".field-upload-audio", function (e) {
      e.preventDefault();
      var button = $(this);
      var inputId =
        button.data("input-id") ||
        button.closest(".cyberisho-field").find("input").attr("name");
      var input = $('input[name="' + inputId + '"]');
      var audioContainer = button.closest(".flex").find(".field-audio-container");
      var deleteLink = button.closest(".flex").find(".field-delete-audio");
  
      var frame = wp.media({
        title: "انتخاب فایل صوتی",
        button: { text: "استفاده از فایل صوتی" },
        multiple: false,
        library: {
          type: ["audio/mpeg", "audio/wav"], // فقط فایل‌های صوتی مجاز
        },
      });
    ...
    // Delete Single Audio
    $(document).on("click", ".field-delete-audio", function (e) {
      e.preventDefault();
      var button = $(this);
      var inputId =
        button.data("input-id") ||
    ...
    $(document).on("click", ".field-upload-img.multiple", function (e) {
      ...
      frame.on("select", function () {
        ...
        attachments.forEach(function (attachment) {
          imgContainer.append(
            '<div class="field-img-item" data-id="' +
              attachment.id +
              '"><img src="' +
              attachment.url +
              '" alt="" style="max-width: 100px; max-height: 100px;" /><a href="#" class="field-delete-img" data-id="' +
              attachment.id +
              '">حذف</a></div>'
          );
        });
    ...
    $(document).on("click", ".field-delete-img[data-id]", function (e) {
      ...
      var inputId =
        button
          .closest(".field-multiple-image-uploader")
          .find(".field-upload-img")
          .data("input-id") ||
        button.closest(".cyberisho-field").find("input").attr("name");
      var input = $('input[name="' + inputId + '"]');
      ...
      $.each(settings, function (key, setting) {
        ...
        if (type === "text") {
          html +=
            '<div id="' +
            fieldId +
            '" class="cyberisho-field field-text">' +
            '<label for="' +
            fieldId +
            '">' +
            title +
          ...
          html +=
            '<div id="' +
            fieldId +
            '" class="cyberisho-field field-textarea">' +
            '<label for="' +
            fieldId +
            '">' +
            title +
          ...
          html +=
            '<div id="' +
            fieldId +
            '" class="cyberisho-field field-image-uploader">' +
            '<label for="' +
            fieldId +
            '">' +
            title +
            ...
            '<button type="button" class="button field-upload-img" data-input-id="' +
            fieldId +
            '">انتخاب تصویر</button>' +
            '<input value="" type="text" class="field-img-url" name="' +
            fieldId +
            '" readonly />' +
            '<div class="field-img-container"></div>' +
            '<a class="field-delete-img hidden" href="#" data-input-id="' +
            fieldId +
            '">حذف تصویر</a>' +
            "</div></div>";
        } else if (type === "audio-uploader") {
          html +=
            '<div id="' +
            fieldId +
            '" class="cyberisho-field field-audio-uploader">' +
            '<label for="' +
            fieldId +
            '">' +
            title +
            "</label>" +
            '<div class="flex align-items-center">' +
            '<button type="button" class="button field-upload-audio" data-input-id="' +
            fieldId +
            '">انتخاب ویس</button>' +
            '<input value="" type="text" class="field-audio-url" name="' +
            fieldId +
            '" readonly />' +
            '<div class="field-audio-container"></div>' +
            '<a class="field-delete-audio hidden" href="#" data-input-id="' +
            fieldId +
            '">حذف ویس</a>' +
            "</div></div>";
        } else if (type === "select") {
          html +=
            '<div id="' +
            fieldId +
            '" class="cyberisho-field field-select">' +
            '<label for="' +
            fieldId +
            '">' +
            title +
            "</label>" +
            '<select class="cyberisho-select select-single" name="' +
            fieldId +
            '">';
