<?php get_header(); ?>
<?php get_template_part('partials/index/header-information-section', 'header-information-section') ?>
<?php get_template_part('partials/index/client-comment-section', 'client-comment-section') ?>
<?php get_template_part('partials/index/about-us-section', 'about-us-section') ?>
<?php get_template_part('partials/index/our-services-section', 'our-services-section') ?>
<?php get_template_part('partials/index/meeting-section', 'meeting-section') ?>
<?php get_template_part('partials/index/portfolio-grid-section', 'portfolio-grid-section') ?>
<?php get_template_part('partials/index/cyberisho-reasons-section', 'cyberisho-reasons-section') ?>
<?php get_template_part('partials/index/accordion-fag-section', 'accordion-fag-section') ?>
<?php get_template_part('partials/index/latest-blog-main-section', 'latest-blog-main-section') ?>
<?php get_footer(); ?>